...
char* createGenshinJson()
{
	...
	cJSON_AddItemToObject(pool_template, "pool_background", cJSON_CreateString(""));
...
GachaPool parse_pool(cJSON* pool_obj) {
	...
	cJSON* background = cJSON_GetObjectItem(pool_obj, "pool_background");
	if (cJSON_IsString(background))
		strncpy_s(pool.pool_background, sizeof(pool.pool_background), background->valuestring, sizeof(pool.pool_background) - 1);
